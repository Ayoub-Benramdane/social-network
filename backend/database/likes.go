...
func LikeGroupPost(user_id, group_id int64, post structs.Post) (int64, error) {
	...
	if !post.IsLiked {
		notification = true
		_, err = DB.Exec("INSERT INTO group_post_likes (user_id, post_id) VALUES (?, ?)", user_id, post.ID)
	} else {
		_, err = DB.Exec("DELETE FROM group_post_likes WHERE user_id = ? AND post_id = ?", user_id, post.ID)
	}
	if err != nil {
		return 0, err
	}
