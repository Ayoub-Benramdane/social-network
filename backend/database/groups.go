...
func GetSuggestedGroups(user_id, offset int64) ([]structs.Group, error) {
	var groups []structs.Group
	rows, err := DB.Query("SELECT g.id, g.name, g.description, g.image, g.cover, g.admin, g.created_at, u.username, g.members FROM groups g JOIN users u ON u.id = g.admin WHERE g.id NOT IN (SELECT group_id FROM group_members WHERE user_id = ? UNION SELECT group_id FROM invitations_groups WHERE invited_id = ?) ORDER BY g.created_at DESC LIMIT ? OFFSET ?", user_id, user_id, 10, offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func GetPendingGroups(user_id, offset int64) ([]structs.Group, error) {
	var groups []structs.Group
	rows, err := DB.Query("SELECT g.id, g.name, g.description, g.image, g.cover, g.admin, g.created_at, u.username, g.members FROM groups g JOIN users u ON u.id = g.admin JOIN invitations_groups i ON g.id = i.group_id WHERE i.invited_id = ? ORDER BY g.created_at DESC LIMIT ? OFFSET ?", user_id, 10, offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
