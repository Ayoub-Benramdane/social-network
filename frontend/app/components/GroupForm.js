...
const GroupForm = () => {
  ...
  const handleEventSubmit = async (groupId) => {
    if (!eventName || !eventDescription || !eventLocation || !eventStartDate || !eventEndDate) {
...
{selectedGroup.posts && selectedGroup.posts.length > 0 && (
  <div style={{ marginTop: 40, padding: "10px", background: "#f0f4f8", borderRadius: "8px" }}>
    <h3 style={{ borderBottom: "2px solid #ccc", paddingBottom: "5px" }}>📌 Group Posts</h3>
    {selectedGroup.posts.map((post) => (
      ...
      >
        <h4>{post.title}</h4>
        <p>{post.content}</p>
        <p><strong>🏷️ Category:</strong> {post.category}</p>
        <p><strong>✍️ By:</strong> {post.author_username || "Unknown"}</p>
...
{selectedGroup.events && selectedGroup.events.length > 0 && (
      <div style={{ marginTop: 40, padding: "10px", background: "#f5f5f5", borderRadius: "8px" }}>
        <h3 style={{ borderBottom: "2px solid #ccc", paddingBottom: "5px" }}>📅 Upcoming Events</h3>
        {selectedGroup.events.map((event) => (
          ...
          >
            ...
            <p>
              <strong>📅 From:</strong> {new Date(event.start_date).toLocaleDateString()} <br />
              <strong>📅 To:</strong> {new Date(event.end_date).toLocaleDateString()}
            </p>
            {event.image && (
              <img
                src={event.image}
